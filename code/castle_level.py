...
PLAYER_SPAWN = (7 * TILE + TILE // 2, 20 * TILE + TILE // 2)

# Spawn boss: riga 3 col 7 — sala del boss in cima
BOSS_SPAWN   = (7 * TILE + TILE // 2,  3 * TILE + TILE // 2)
...
class CastleLevel:
		...
		for row in range(MAP_ROWS):
			for col in range(MAP_COLS):
				...
				if cell == 1:
					...
					CastleTile(pos, floor_tiles[variant],
					           [self.visible_sprites], is_floor=True)

				elif cell == 2:
					CastleTile(pos, floor_tiles[0],
					           [self.visible_sprites], is_floor=True)
					variant = rng.randint(0, 3)
					CastleTile(pos, wall_tiles[variant],
			...
			for enemy in list(self.enemy_sprites):
				if enemy not in self.player.hit_this_attack:
					if self.player.weapon_sprite.rect.colliderect(enemy.hitbox):
						...
						# is_dead diventa True subito quando HP <= 0
						if getattr(enemy, 'is_dead', False) and enemy not in self._defeated_enemies:
							self._defeated_enemies.add(enemy)
	...
	def _on_boss_defeated(self):
		...
		eroe = (f'eroina {self.player_name} {PLAYER_SURNAME}'
		        if gender == 'female'
		        else f'eroe {self.player_name} {PLAYER_SURNAME}')

		self.dialogue_ui.open([
			'Il Guardiano di Casteddu è caduto!',
	...
	def run(self, events=[]):
		...
		# ── Barra HP boss (disegnata sopra gli sprite, sotto HUD) ────────
		if self.boss.alive():
			self.boss.draw_boss_ui(self.screen, self.visible_sprites.offset)
