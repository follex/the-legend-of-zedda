...
class CharacterSelect:
    ...
    def draw(self):
        ...
        self.screen.blit(title, title.get_rect(centerx=WIDTH // 2, y=40))

        sub = self.font_small.render('Scegli il tuo personaggio', True, '#aaaaaa')
        self.screen.blit(sub, sub.get_rect(centerx=WIDTH // 2, y=120))

        label_g = self.font_small.render('— GENERE —', True, '#c8a96e')
        self.screen.blit(label_g, label_g.get_rect(centerx=WIDTH // 2, y=190))

        gx_positions = [WIDTH // 2 - 160, WIDTH // 2 + 160]
        ...
        self.screen.blit(label_r, label_r.get_rect(centerx=WIDTH // 2, y=340))

        rx_positions = [WIDTH // 2 - 280, WIDTH // 2, WIDTH // 2 + 280]
        ...
        if self.selected_gender and self.selected_role:
            self.start_button_rect = self.draw_button('▶  INIZIA', WIDTH // 2, 540, selected=True)
        else:
            hint = self.font_small.render('Seleziona genere e ruolo per iniziare', True, '#666666')
            self.screen.blit(hint, hint.get_rect(centerx=WIDTH // 2, y=530))
